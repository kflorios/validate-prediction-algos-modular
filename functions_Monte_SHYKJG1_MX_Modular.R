...
fc_nnet <- function(X,y) {

  p.nnet <- nnet(X,y, size = 4, decay = 10^(-1),
                 entropy=T,maxit=2000,MaxNWts=2000  )
  return(p.nnet)
}

fc_nnet_predict <- function(mObject,Xnew) {
  newData <- as.data.frame(Xnew)
  p.nnet.predict <- predict(mObject,newData)
  return(p.nnet.predict)
}

fc_svm <- function(X,y) {
  p.svm <- svm(X,y, gamma=0.5, cost=8, probability=TRUE)
  return(p.svm)  
}
...
fc_randomForest <- function(X,y) {
  ...
  data <- as.data.frame(cbind(X,y))
  p.randomForest <- randomForest( y ~ ., data = data, importance=TRUE, na.action=na.omit)
...
evaluateModels <- function(fname,m) {
  ...
  ###m1 <- lm( yGTM1 ~ logRBr+FSPIBr+TLMPILBr+DIBr+BeffBr+IsinEn1Br+IsinEn2Br+NNC,data=te)
  ###summary(m1)
     X = cbind(te$logRBr,te$FSPIBr,te$TLMPILBr,te$DIBr,te$BeffBr,te$IsinEn1Br,te$IsinEn2Br,te$NNC)
     y = te$yGTM1
     m1 <- fc_lm(X,y)
     summary(m1)
  ...
  outD <- cbind("Param" = rep(paste(mychar[1:9],sep = " "), len = nrow(outD)), outD)
  ...
  #m2 <- glm( yGTM1 ~ logRBr+FSPIBr+TLMPILBr+DIBr+BeffBr+IsinEn1Br+IsinEn2Br+NNC,data=te,family=binomial(probit))
  ...
  #m3 <- glm( yGTM1 ~ logRBr+FSPIBr+TLMPILBr+DIBr+BeffBr+IsinEn1Br+IsinEn2Br+NNC,data=te,family=binomial(logit))
  ...
  #teRF <- rbind(te,teTst)
  #trainRF <- 1:dim(te)[1]
  #p.randomForest <- randomForest(yGTM1 ~ logRBr+FSPIBr+TLMPILBr+
  #                                          DIBr+BeffBr+IsinEn1Br+IsinEn2Br+NNC, data=teRF, subset = trainRF,
  #                                            importance=TRUE, na.action=na.omit)
  
  require(randomForest)
  X = cbind(te$logRBr,te$FSPIBr,te$TLMPILBr,te$DIBr,te$BeffBr,te$IsinEn1Br,te$IsinEn2Br,te$NNC)
  y = te$yGTM1
  p.randomForest <- fc_randomForest(X,y)  
  plot(p.randomForest)
  ...
  #p.svm <- svm(cbind(te$logRBr,te$FSPIBr,te$TLMPILBr,te$DIBr,te$BeffBr,te$IsinEn1Br,te$IsinEn2Br,te$NNC),
  ...
  #t.svm <- tune.svm(cbind(te$logRBr,te$FSPIBr,te$TLMPILBr,te$DIBr,te$BeffBr,te$IsinEn1Br,te$IsinEn2Br,te$NNC),
  #                  te$yGTM1,gamma = 2^(-2:0), cost = 2^(0:2), probability=TRUE)

  #p.svm <- t.svm$best.model
  
  #t.svm <- tune.svm(cbind(te$logRBr,te$FSPIBr,te$TLMPILBr,te$DIBr,te$BeffBr,te$IsinEn1Br,te$IsinEn2Br,te$NNC),
  #                  te$yGTM1,gamma = 0.5, cost = 4, probability=TRUE)
  ...
  #p.svm <- svm(cbind(te$logRBr,te$FSPIBr,te$TLMPILBr,te$DIBr,te$BeffBr,te$IsinEn1Br,te$IsinEn2Br,te$NNC),
  #             te$yGTM1,gamma = 0.5, cost = 8, probability=TRUE)
  ...
  #for(tries in 1:1) {
    ...
    #for (iNode in 16:16) {
      ...
      ###p.nnet <- nnet(cbind(te$logRBr,te$FSPIBr,te$TLMPILBr,te$DIBr,te$BeffBr,te$IsinEn1Br,te$IsinEn2Br,te$NNC),
      ...
      #t.nnet <- tune.nnet(cbind(te$logRBr,te$FSPIBr,te$TLMPILBr,te$DIBr,te$BeffBr,te$IsinEn1Br,te$IsinEn2Br,te$NNC),
      #                    te$yGTM1, size = 8*2^(-1:1), decay = 10^(-3:-1),
      #                    entropy=T,maxit=2000,MaxNWts=2000  )
      ...
      Xnew <- as.matrix(teTst[,-c(9,10)])
      p.nnet.predict <- fc_nnet_predict(p.nnet,Xnew)
      ...
      for (thresHOLD in seq(0.00,1.00,0.01) )
      {
        ...
        #Xnew <- as.matrix(teTst[,-c(9,10)])
        Xnew <- teTst[,-c(9,10)]
        names(Xnew) <- c("V1","V2","V3","V4","V5","V6","V7","V8")
        Xnew <- as.matrix(Xnew)
        m1.predict <- fc_lm_predict(m1,Xnew)  
        ...
        names(Xnew) <- c("V1","V2","V3","V4","V5","V6","V7","V8")
        Xnew <- as.matrix(Xnew)
        m2.predict <- fc_probit_predict(m2,Xnew)  
        ...
        names(Xnew) <- c("V1","V2","V3","V4","V5","V6","V7","V8")
        Xnew <- as.matrix(Xnew)
        m3.predict <- fc_logit_predict(m3,Xnew)  
        ...
        names(Xnew) <- c("V1","V2","V3","V4","V5","V6","V7","V8")
        Xnew <- as.matrix(Xnew)
        p.randomForest.predict <- fc_randomForest_predict(p.randomForest,Xnew)
        ...
        Xnew <- as.matrix(teTst[,-c(9,10)])
        p.svm.predict <- fc_svm_predict(p.svm,Xnew)
